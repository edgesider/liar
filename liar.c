...
void on_connect_enter(struct child_info *ci) {
  ...
  if (connect(fdp, (struct sockaddr *)&proxyaddr, sizeof(proxyaddr)) == -1) {
    ...
    ci->curr_err = 0;
    int err =
        socks4_connect(fdp, ((struct sockaddr_in *)origaddr)->sin_addr.s_addr,
...
void stopped(struct child_info *ci) {
  ...
  if (sig != SIGTRAP) {
    if (sig == SIGCHLD) {
      ...
      logd("[child stopped by signal (ignored): %s]",
           strsignal(WSTOPSIG(ci->status)));
      return;
    }
  }
...
int wait_child(struct child_info *ci) {
  ...
  if (WIFSTOPPED(ci->status)) {
    ...
    ci->curr_call = -1;
    erre_sys(
        ptrace(PTRACE_SETOPTIONS, ci->pid, 0,
  ...
  for (;;) {
    ...
    if (WIFEXITED(ci->status)) {
      ...
      logd("child exited [signal: %s]: %d", strsignal(WSTOPSIG(ci->status)),
           WEXITSTATUS(ci->status));
      return 0;
